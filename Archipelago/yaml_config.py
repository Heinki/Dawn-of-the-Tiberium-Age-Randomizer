...
def serialize_player_yaml(manifest, slot_name):
    ...
    output = (
        ...
        f"game: {GAME_NAME}\n"
        "description: Dawn of the Tiberium Age Randomizer generated player file\n"
        "requires:\n"
        "  version: 0.6.7\n\n"
        f"{GAME_NAME}:\n"
    )
...
def parse_player_yaml(text):
    ...
    if not manifest_parts:
        raise ValueError("Player YAML has no Dawn of the Tiberium Age generated_world data.")
    try:
