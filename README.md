...
- Adds unit-specific production-clone buffs where the installed DTA rules support them: production, cost, speed, armor, health, damage, reload, range, blast area, sight, ammo, passenger capacity, cloaking, sensors, staged regeneration, and amphibious movement for dry-land vehicles. Regeneration access first adds `SELF_HEAL` at Veteran rank, then enables self-healing from Rookie rank. Separate upgrades raise the healing cap from 50% to 100% or shorten the interval between healing ticks; neither changes `SelfHealingStep`. Natural self-healers receive only the cap and rate upgrades.
...
- Exposes 123 canonical mobile types: 24 infantry, 91 vehicles/naval units, and 8 aircraft. The 111 non-essential types can become access rewards. Engineer, both resource harvesters, Chinook, all four MCVs, and all four faction hovercraft transports remain permanently available. Each MCV requires its matching War Factory/Airstrip, so capturing an enemy factory also grants access to that faction's MCV.
...
- Always provides the current faction's basic power plant and refinery during randomized unit access. Earned infantry, vehicles, and aircraft also unlock their matching Barracks, War Factory/Airstrip, or air production building without technology prerequisites. Earned ships unlock a naval yard only when the mission naturally enables that yard; dry maps no longer gain unusable naval infrastructure. Chaos, Arsenal, and Shop earned units can use compatible factories from every faction. A deployed MCV/Construction Yard is still required to construct buildings.
- Runs Toxic Diversion with Covert Revolt's enhanced DTA mode and enhanced AI-discount base. Map-local clones flatten the complete `Rules.ini` plus `Enhance.ini` runtime stack, preserving enhanced unit, vehicle, weapon, art, and prerequisite changes.
...
- Includes 16 defensive-building unlocks and their supported production, cost, durability, weapon, range, sight, cloaking, sensor, and staged self-healing buffs. Hidden DTA defenses include Allied Artillery Emplacement, Land Mine, Gap Generator, and Soviet Shore Artillery.
- Offers recharge-speed, damage, and blast-radius rewards for the Ion Cannon. The player receives a map-local clone of the native `IonCannon` engine type. While active, native providers, grants, and scripted Ion strikes are removed and the native power is recharge-locked, so only the player clone can fire the buffed global Ion effect. The neutral Paradrop remains available to every faction and gains recharge speed plus selectable infantry and light-vehicle reinforcements. Shop Mode lists each reinforcement separately and prices it by unit strength.
...
Amphibious Drive copies the working HVCSAM Hover MLRS movement combination:
Drive `Locomotor`, `SpeedType=Hover`, and `MovementZone=Fly`. It is offered
only for dry-land vehicles, never naval units, already water-capable vehicles,
infantry, or aircraft. Keep all three movement fields together; the retired
two-field implementation did not work reliably in DTA.
