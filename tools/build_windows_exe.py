...
The driver mirrors build_exe.ps1 and can run through Windows Python in Wine,
allowing Linux maintainers to keep producing the normal Windows executable.
"""
...
def prepare_tcl_bundle(source: Path, destination: Path) -> None:
    ...
    original_path = destination / '_dta_original_init.tcl'
    init_path.replace(original_path)
    init_path.write_text(
        '# DTA Randomizer bundled Tcl bootstrap\n'
        'set ::tcl_library [file dirname [info script]]\n'
        'source [file join $::tcl_library _dta_original_init.tcl]\n',
        encoding='utf-8',
    )
...
def build(output: Path) -> None:
    ...
    icon = require_path(PROJECT_ROOT / 'launcher_icon.ico', 'Launcher icon')
    puzzle = require_path(PROJECT_ROOT / 'DTA Puzzle.png', 'DTA puzzle image')
    ...
    with tempfile.TemporaryDirectory(prefix='dta-windows-build-') as temporary:
        ...
        built = require_path(dist / 'DTARandomizer.exe', 'Built launcher')
        archive = run_checked(
            [
                sys.executable,
                '-m',
...
def main() -> int:
    ...
    parser.add_argument(
        '--output',
        type=Path,
        default=PROJECT_ROOT.parent / 'DTARandomizer.exe',
    )
