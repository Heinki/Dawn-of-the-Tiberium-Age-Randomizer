...
def _activate_dta_reward_catalogue():
    """Replace the imported MO catalogue with verified DTA-wide modifiers.

    Unit-specific cloning remains disabled until its map collision/routing
    implementation can satisfy the project's map-object preservation rule.
    """
    global BUFF_TARGETS, BUFF_TYPES, UNIT_BUFF_REWARDS, REWARD_POOL
    global REWARD_BY_NAME, REWARD_BY_BUFF_KEY, REWARD_ALIASES
    global UNIT_UNLOCK_REWARDS, EXTRA_UNIT_UNLOCK_REWARDS
    global ROSTER_UNIT_UNLOCK_REWARDS, DEFENSE_UNLOCK_REWARDS
    global SPECIAL_BUILDING_UNLOCK_REWARDS, SUPERWEAPON_UNLOCK_REWARDS
    global SECONDARY_SUPERWEAPON_UNLOCK_REWARDS, AID_POWER_UNLOCK_REWARDS
    global POWER_BUFF_REWARDS, ENEMY_REWARD_POOL, SPECIAL_BUILDING_DEFINITIONS
    global FACTION_UNIT_ROSTERS, FACTION_DEFENSE_ROSTERS, UNIT_LABELS
    global NAVAL_UNIT_IDS, ALWAYS_AVAILABLE_UNIT_IDS, ALWAYS_AVAILABLE_TECH_IDS
    global ENGINEER_UNIT_IDS, AMPHIBIOUS_TRANSPORT_UNIT_IDS
    global LIMITED_HERO_UNIT_IDS, STANDALONE_WEAPON_TEMPLATES

    factions = ['GDI', 'Nod', 'Allies', 'Soviet']
    target_id = 'DTA_PLAYER_ARMY'
    BUFF_TARGETS = {
        target_id: {
            'label': 'Player Army',
            'plural': 'player units',
            ...
            'global_buff': True,
            'allowed_buff_types': [
                'damage', 'armor', 'speed', 'reload', 'cost', 'production',
            ],
        },
    }
    supported = {'damage', 'armor', 'speed', 'reload', 'cost', 'production'}
    BUFF_TYPES = [item for item in BUFF_TYPES if item.get('id') in supported]
    UNIT_BUFF_REWARDS = [
        {
            'name': f'Player Army {buff_type["name"]} I',
            'description': (
                'Applies a DTA/Vinifera human-house modifier. '
                'Enemy units and map object identities stay unchanged.'
            ),
            'rules': {},
            'factions': factions,
            'kind': 'buff',
    ...
    REWARD_POOL = list(UNIT_BUFF_REWARDS)
    ...
    UNIT_LABELS = {target_id: 'Player Army'}
    NAVAL_UNIT_IDS = set()
    ALWAYS_AVAILABLE_UNIT_IDS = set()
    ALWAYS_AVAILABLE_TECH_IDS = set()
...
_activate_dta_reward_catalogue()
