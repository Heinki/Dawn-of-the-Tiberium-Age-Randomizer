...
def apply_color_mode(self):
    ...
    shop_difficulty_colors = {
        'Easy': '#7ee787' if self.dark_mode_var.get() else '#1a7f37',
        'Normal': '#79c0ff' if self.dark_mode_var.get() else '#0969da',
        'Hard': '#ffa657' if self.dark_mode_var.get() else '#bc4c00',
        'Brutal': '#ff7b72' if self.dark_mode_var.get() else '#cf222e',
        'Extreme': '#f778ba' if self.dark_mode_var.get() else '#bf3989',
        'Ultimate': '#d2a8ff' if self.dark_mode_var.get() else '#8250df',
        'Impossible': '#f0f6fc' if self.dark_mode_var.get() else '#24292f',
    }
    ...
    # Combobox popdowns are Tk Listboxes, so ttk style colors do not reach them.
    ...
    for canvas_name in ('settings_canvas', 'grid_canvas', 'shop_canvas'):
