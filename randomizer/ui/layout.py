...
def _build_window_shell(self):
    ...
    header = ttk.Label(
        main_frame,
        text=f'DTA Randomizer Launcher v{APP_VERSION}',
        font=('Segoe UI', 14, 'bold'),
    )
    ...
    self.copy_seed_button = ttk.Button(
        main_frame, text='Copy Seed', command=self.copy_active_seed,
        state='disabled',
    )
    ...
    self.missions_tree.column('faction', width=135, anchor='w', stretch=False)
...
def _build_right_panel(self, main_frame):
    ...
    ttk.Label(seed_settings_frame, text='Seed name (optional; blank generates a new seed)', font=('Segoe UI', 10, 'bold')).grid(row=0, column=0, sticky='w')
    ...
    ttk.Label(
        ...
        text=(
            ...
            'for that stage. Each '
            'selected mission gets one attempt. An Emergency Revival repeats '
            'a failed stage with new mission choices.'
        ),
        style='Muted.TLabel',
        justify='left',
        wraplength=720,
...
def _build_info_tabs(self, info_tabs):
    ...
    for faction in ('GDI', 'Nod', 'Allies', 'Soviet', 'Neutral'):
    ...
    info_tabs.add(enemy_buffs_page, text='Enemy Buffs')
    ttk.Label(
        enemy_buffs_page,
        text='Acquired and applied enemy buffs',
        font=('Segoe UI', 10, 'bold'),
        justify='left',
    ...
    ttk.Label(
        enemy_buffs_page,
        text=(
            'Assigned buffs appear immediately. Applied cards confirm a safe '
            'target in the generated mission; no-build missions apply none.'
        ),
        style='Muted.TLabel',
        wraplength=820,
        justify='left',
