...
def _append_advanced_common_actions(self, parent, pool_key):
    ...
    ttk.Button(
        parent, text='Include', width=7, padding=(6, 3),
        command=lambda key=pool_key: self.set_advanced_pool_all(key, True),
    ).pack(side='left', padx=(0, 4))
    ttk.Button(
        parent, text='Exclude', width=7, padding=(6, 3),
...
def _append_advanced_selected_actions(
    self, parent, pool_key, faction_var, category_var,
):
    ttk.Label(parent, text='Selected:').pack(side='left', padx=(0, 4))
    ttk.Button(
        parent, text='Include', width=7, padding=(6, 3),
        command=lambda key=pool_key: self.set_advanced_pool_groups(
            key, faction_var.get(), category_var.get(), True
        ),
    ).pack(side='left', padx=(0, 4))
    ttk.Button(
        parent, text='Exclude', width=7, padding=(6, 3),
        command=lambda key=pool_key: self.set_advanced_pool_groups(
            key, faction_var.get(), category_var.get(), False
        ),
    ).pack(side='left')
...
def _build_advanced_tab(self, workspace_tabs):
    ...
    for pool_key, pool_label in (
        ...
        if pool_key == 'missions':
            ...
            ttk.Label(row, text='Campaign:').pack(side='left', padx=(0, 4))
            campaign_var = tk.StringVar(value=CAMPAIGN_FILTERS[0])
            self.advanced_pool_group_vars[pool_key] = {
                'campaign': campaign_var,
            }
            ttk.Combobox(
                row,
                state='readonly',
                textvariable=campaign_var,
                values=tuple(CAMPAIGN_FILTERS),
                width=23,
            ).pack(side='left', padx=(0, 4))
            ttk.Button(
                row,
                text='Only',
                command=lambda key=pool_key, target=campaign_var: (
                    self.set_advanced_pool_only(key, target.get())
                ),
            ).pack(side='left', padx=(0, 4))
            ttk.Button(
                row,
                text='Include',
                command=lambda key=pool_key, target=campaign_var: (
                    self.set_advanced_pool_group(
                        key, 'campaign', target.get(), True
                    )
                ),
            ).pack(side='left', padx=(0, 4))
            ttk.Button(
                ...
                command=lambda key=pool_key, target=campaign_var: (
                    self.set_advanced_pool_group(
                        key, 'campaign', target.get(), False
                    )
                ),
            ).pack(side='left')
            _append_advanced_common_actions(self, row, pool_key)
        else:
            ...
            for column, (value, label) in enumerate(faction_choices, start=1):
                ttk.Radiobutton(
                    selector_frame,
                    text=label,
                    value=value,
            ...
            for column, (value, label) in enumerate(category_choices, start=1):
                ttk.Radiobutton(
                    ...
                    row=1, column=column, sticky='w', padx=(0, 2),
                    pady=(6, 0),
                )
            ...
            _append_advanced_selected_actions(
                self, action_row, pool_key, faction_var, category_var,
            )
        canvas = tk.Canvas(
            page,
            borderwidth=0,
    ...
    for index, buff_type in enumerate(BUFF_TYPES):
        ...
        option.grid(
            row=index // 2, column=index % 2, sticky='ew', padx=(0, 8)
        )
...
def _build_gameplay_settings(self, settings_frame):
    ...
    self.randomize_unit_access_check = ttk.Checkbutton(
        reward_frame,
        text='Randomize mobile-unit access and lock unearned units',
