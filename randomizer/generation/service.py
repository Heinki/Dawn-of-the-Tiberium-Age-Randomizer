...
from randomizer.missions.catalogue import (
    filter_missions_by_build_settings, mission_matches_campaign,
)
...
class RunGenerator(SeedGeneration, RewardGeneration, GenerationSettings, StartingUnlocks):
    ...
    def generate(self, seed):
        ...
        missions = [mission for mission in self.missions
                    if mission['code'].upper() not in excluded
                    and (campaign == CAMPAIGN_FILTERS[0]
                         or mission_matches_campaign(mission, campaign))]
        ...
        for key, maximum in (('mission_goal', len(self.missions)),
        ...
        self._seed_generation_context = {
            ...
            'use_act_based_reward_multipliers': config['use_act_based_reward_multipliers'],
        }
        settings = self.config_reward_settings()
        if reward_mode == ARSENAL_MODE:
            settings.update(randomize_unit_access=False, start_with_tier_one_units=False,
                            start_with_tier_one_defenses=False, starting_reward_count=0,
                            starting_unlock_rewards=[])
            if not settings['arsenal']['factions']:
        ...
        # Wall-clock time must not change slot data for an identical AP seed.
