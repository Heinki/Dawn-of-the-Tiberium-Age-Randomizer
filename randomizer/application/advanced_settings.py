...
class AdvancedSettingsController:
        ...
        for reward in REWARD_POOL:
            ...
            for unit_id in tech_ids_for_rewards([reward]):
                ...
                entry = entries.setdefault(unit_id, {
                    ...
                    'category': (
                        'units'
                        if target.get('category') == 'vehicles'
                        else target.get('category')
                    ),
    ...
    def advanced_buff_unit_is_visible(self, entry, included_only=True):
        ...
        selected_factions = campaign_factions(self.missions, selected_campaign)
        return (
    ...
    def advanced_power_pool_entries(self):
        ...
        faction_rank = {'GDI': 0, 'Nod': 1, 'Allies': 2, 'Soviet': 3, 'Other': 4}
        return sorted(
            entries.values(),
            key=lambda entry: (
                faction_rank.get(entry['faction'], 4),
    ...
    def set_advanced_pool_all(self, pool_key, include):
        ...
        all_ids = {
            ...
            if self.advanced_pool_entry_is_visible(entry, pool_key)
        }
        if include:
    ...
    def advanced_pool_bulk_entries(self, pool_key):
        """Return eligible entries and their existing exclusion set."""
        ...
        if pool_key == 'units':
            entries = [
                {**entry, 'special': bool(entry.get('special_reward'))}
        ...
        entries = [
            ...
            if entry['category'] in enabled_categories
            and (
                self.include_special_rewards_var.get()
                or not entry['reward'].get('special_reward')
            )
    ...
    def advanced_pool_group_choices(self, pool_key):
        """Build faction/category choices from DTA catalogue metadata."""
        if pool_key == 'units':
        ...
        choices = [
            ...
            for faction in ('GDI', 'Nod', 'Allies', 'Soviet', 'Neutral', 'Other')
    ...
    def advanced_pool_entry_is_visible(self, entry, pool_key):
        """Match campaign and Arsenal scope used by Advanced cards."""
        selected_campaign = self.campaign_var.get()
        if pool_key == 'missions':
            return (
                selected_campaign == CAMPAIGN_FILTERS[0]
                or entry.get('campaign') == selected_campaign
            )
        if self.reward_mode_var.get() == ARSENAL_MODE:
            arsenal_factions = {
                faction
                for faction, variable in self.arsenal_faction_vars.items()
                if variable.get()
            }
            return (
                entry.get('faction') == 'Neutral'
    ...
    def set_advanced_pool_only(self, pool_key, selector):
        """Include selected campaign/special group; exclude other scoped entries."""
        if self.gameplay_settings_locked():
            return
        entries, target = self.advanced_pool_bulk_entries(pool_key)
        scope = [
        ...
        elif pool_key == 'missions':
            included_ids = {
                entry['id']
                for entry in scope
                if selector == CAMPAIGN_FILTERS[0]
                or entry.get('campaign') == selector
            }
        else:
            included_ids = {
                entry['id'] for entry in scope
    ...
    def set_advanced_pool_group(self, pool_key, group_type, value, include):
        """Bulk-toggle one campaign/faction/category without other changes."""
        if self.gameplay_settings_locked():
            return
        entries, target = self.advanced_pool_bulk_entries(pool_key)
        scope = [
        ...
        elif group_type == 'campaign' and value == CAMPAIGN_FILTERS[0]:
            matching_ids = {entry['id'] for entry in scope}
        else:
            matching_ids = {
                entry['id'] for entry in scope
    ...
    def set_advanced_pool_groups(
        self, pool_key, faction, category, include,
    ):
        """Bulk-toggle one faction/category intersection."""
        if self.gameplay_settings_locked():
            return
        entries, target = self.advanced_pool_bulk_entries(pool_key)
        matching_ids = {
    ...
    def on_unlimited_hero_units_changed(self):
        ...
        if self.unlimited_hero_units_var.get() and build_limit_var is not None:
    ...
    def refresh_setting_states(self):
        ...
        reward_source_enabled = (
            bool(self.randomize_unit_access_var.get()) or buffs_enabled
        )
