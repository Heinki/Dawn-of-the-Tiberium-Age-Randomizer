...
class LaunchController:
    ...
    def read_spawn_difficulty(self):
        if SPAWN_INI.exists():
            try:
                for line in SPAWN_INI.read_text(
                    encoding='utf-8', errors='ignore'
                ).splitlines():
                    key, separator, value = line.partition('=')
                    if separator and key.strip().casefold() == 'difficultyname':
                        selected = value.strip()
                        for label, _ in DIFFICULTIES:
                            if label.casefold() == selected.casefold():
                                return label
            except OSError:
                pass
        return 'Normal'
...
"""
    ...
    def write_spawn_ini(self, mission, difficulty, game_speed_value):
        try:
            ...
            options = self.spawn_reward_options()
            options.update({
                'Side': mission.get('side', 0),
                'Firestorm': 'True' if mission.get('required_addon') else 'False',
    ...
    def write_launch_options(self, difficulty_value, game_speed_value):
        # DTA consumes these values from spawn.ini. Its client owns SUN.INI
        # and regenerated client settings, so the randomizer must not patch them.
        return

    def _write_legacy_launch_options(self, difficulty_value, game_speed_value):
        try:
            written = []
            skipped = []
            for path in (OPTIONS_INI, YR_OPTIONS_INI):
                # Do not create option files that the installation does not
                # already use. This compatibility branch is bypassed by DTA;
    ...
    def disable_generated_rules_for_client(self):
        # DTA's generated INI/Rules.ini is client-owned. Never rename, remove,
        # or replace it. All randomizer changes belong in spawnmap.ini.
        return
    ...
    def launch_mission_async(self, mission, extra_rules=None, launch_note=''):
        ...
        if difficulty.used_fallback:
            self.append_log(
                f'{difficulty.requested_label} is unavailable for '
                f'{mission.get("title", mission.get("code", "mission"))}; '
    ...
    def prepare_mission_launch_files(
        ...
        try:
            ...
            orphan_buffs = [
                ...
                if item.get('reason') == 'buff_without_access'
            ]
            if orphan_buffs:
                self.append_log(
                    'Skipped DTA unit buffs without matching production access: '
                    + ', '.join(item['unit'] for item in orphan_buffs)
                    + '. Old/save-carried rewards cannot grant a locked unit.'
                )
            if access_report['enabled']:
                if access_report.get('skipped_reason'):
                    self.append_log(
                        'Skipped DTA production access isolation: '
                        f'{access_report["player_house"]} produces through '
                        f'{access_report["production_house"]} (ActsLike '
                        f'{access_report["acts_like"]}), shared by hostile house(s) '
                        + ', '.join(access_report['shared_hostile_houses'])
                        + '. Vinifera has no exact scenario-house production gate; '
                        'enemy production and authored map identities were preserved.'
                    )
                else:
                    self.append_log(
                        'Applied DTA player-only mobile-unit access through '
                        f'{access_report["production_house"]} (scenario house '
                        f'{access_report["player_house"]}, ActsLike '
                        f'{access_report["acts_like"]}): '
                        f'{len(access_report["clone_unlocked"])} earned player clones '
                        'unlocked, '
                        f'{len(access_report["locked"])} original types locked. '
            ...
            if existing_log is not None:
                ...
                hook['offset'] = existing_log.stat().st_size
            self.write_spawn_ini(mission, difficulty, game_speed_value)
            self.write_launch_options(difficulty.engine_value, game_speed_value)
        except Exception:
            self.cleanup_generated_root_maps()
            self.disable_generated_rules_for_client()
            raise
