...
class UnlockViewController:
    ...
    def refresh_unlock_dashboard(self):
        ...
        signature = (
            ...
            tuple(
                (
                    ...
                    tuple(
                        (source, reward.get('name'), reward.get('power_buff_type'))
                        for source, reward in entry['sources']['earned']
                    ),
                    tuple(
                        (source, reward.get('name'), reward.get('power_buff_type'))
                        for source, reward in entry['sources']['available']
                    ),
                    tuple(
                        (source, reward.get('name'), reward.get('power_buff_type'))
                        for source, reward in entry['sources']['available_unlocks']
                    ),
        ...
        order = {'Infantry': 0, 'Vehicles': 1, 'Vehicles / Naval': 1,
                 'Naval': 2, 'Aircraft': 3, 'Defenses': 4, 'Special': 5,
                 'Special Buildings': 6, 'Superweapons': 7, 'Global Buffs': 0,
                 'House-Wide Buffs': 1}
    ...
    def refresh_progress_view(self):
        ...
        def reward_detail_tag(reward_value):
            ...
            if reward.get('kind') in {
                None, 'unit_access', 'building_access', 'access',
            }:
