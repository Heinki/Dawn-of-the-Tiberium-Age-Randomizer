...
class ProgressionController:
    ...
    def redraw_progression_views(self):
        ...
        if hasattr(self, 'workspace_tabs') and hasattr(self, 'mission_view_frame'):
            # Shop Mode removes the mission view from the workspace notebook.
            # A progression combobox event still redraws this view afterward,
            # so only rename the tab while the notebook manages it.
            if str(self.mission_view_frame) in self.workspace_tabs.tabs():
    ...
    def redraw_mission_tree(self):
        ...
        for idx, mission in self.visible_missions():
            ...
            self.missions_tree.insert(
                ...
                'end',
                iid=str(idx),
                values=(f'{order:03}', state, checks_label, campaign, code, title),
                tags=tuple(tags),
            )
