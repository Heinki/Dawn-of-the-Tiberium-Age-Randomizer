...
from randomizer.generation.starting_unlocks import StartingUnlocks

class StartingUnlocksController(StartingUnlocks):
    ...
    def starting_unlock_entries(self):
        ...
        faction_rank = {
            'GDI': 0, 'Nod': 1, 'Allies': 2,
            'Soviet': 3, 'Neutral': 4, 'Other': 5,
        }
    ...
    def starting_unlock_visible_for_campaign(self, entry):
        ...
        if selected == CAMPAIGN_FILTERS[0]:
            return True
        selected_factions = campaign_factions(self.missions, selected)
        return bool(factions.intersection(selected_factions | {'Neutral'}))
