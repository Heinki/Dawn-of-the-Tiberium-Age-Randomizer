...
class LauncherApp(
    ...
    def __init__(self):
        super().__init__()
        self.title(f'DTA Randomizer Launcher v{APP_VERSION}')
    ...
    def finish_initial_load(self, missions):
        ...
        if (
            ...
            and self.config.get('generation', {}).get(
                'randomize_unit_access', False
            )
        ):
            self.append_log(
                'Current seed was generated before DTA mobile-unit access was '
                'enabled. It remains a buff-only run. Generate a new seed to '
                'receive infantry unlock rewards.',
                error=True,
            )
