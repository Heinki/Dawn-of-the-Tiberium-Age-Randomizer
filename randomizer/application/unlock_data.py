...
class UnlockDataController:
    ...
    def current_unlocks_text(self):
        ...
        if self.randomize_unit_access_enabled():
            ...
            permanent_ids = sorted(
                (
                    unit_id for unit_id in ALWAYS_AVAILABLE_TECH_IDS
                    if BUFF_TARGETS.get(unit_id, {}).get('category')
                    in {'infantry', 'units', 'vehicles', 'aircraft'}
                ),
                key=self.unit_faction_sort_key,
            )
            lines.append('Always available:')
            for unit_id in permanent_ids:
                factions = BUFF_TARGETS.get(unit_id, {}).get('factions', ())
                lines.append(
                    f'  {unit_display_label(unit_id)} ({unit_id})'
                    + (f' - {" / ".join(factions)}' if factions else '')
                )
            earned_access_ids = set(unlocked_reward_tech_ids(earned))
            earned_access_ids.update(self.active_starting_tier_one_access_ids())
            earned_access_ids.difference_update(ALWAYS_AVAILABLE_TECH_IDS)
            earned_access_ids = {
                unit_id for unit_id in earned_access_ids
                if BUFF_TARGETS.get(unit_id, {}).get('category')
                in {'infantry', 'units', 'vehicles', 'aircraft'}
            }
            lines.append('Earned:')
            if earned_access_ids:
                for unit_id in sorted(
                    earned_access_ids, key=self.unit_faction_sort_key
                ):
                    factions = BUFF_TARGETS.get(unit_id, {}).get('factions', ())
                    lines.append(
                        f'  {unit_display_label(unit_id)} ({unit_id})'
                ...
                lines.append('  None')
            if selected:
                active_access_ids = set(unlocked_reward_tech_ids(
                    self.launch_rewards_for_mission(selected['code'])
                )) - set(ALWAYS_AVAILABLE_TECH_IDS)
                active_access_ids = {
                    unit_id for unit_id in active_access_ids
                    if BUFF_TARGETS.get(unit_id, {}).get('category')
                    in {'infantry', 'units', 'vehicles', 'aircraft'}
                }
                lines.append(
                    'Earned access active in selected mission '
                    f'({selected.get("title", selected["code"])}):'
                )
                if active_access_ids:
                    for unit_id in sorted(
                        active_access_ids, key=self.unit_faction_sort_key
                    ):
                        lines.append(
                            f'  {unit_display_label(unit_id)} ({unit_id})'
                        )
    ...
    def unlock_dashboard_entries(self):
        ...
        for unit_id, target in BUFF_TARGETS.items():
            ...
            unlocked = bool(source_data['earned']) if global_target else (
                ...
                if arsenal_mode
                    ...
                    ('Foehn' not in eligible_factions or foehn_units_available or special_reward)
                    and (
            ...
            for display_faction in display_factions:
                entry_key = (
                    key
                    if len(display_factions) == 1
                    else f'{key}@{display_faction}'
                )
                entries.append({
                    'key': entry_key,
                    'source_key': key,
        ...
        for entry in entries:
            source_key = entry.get('source_key', entry['key'])
            if source_key in starting_unlock_source_by_key:
                entry['condition'] = starting_unlock_source_by_key[source_key]
        for entry in entries:
    ...
    def unlock_dashboard_tooltip(self, entry):
        ...
        if entry.get('production_building_role'):
            lines.append(
                f'Factory support: {entry["production_building_role"]} '
                '(mission faction only), prerequisite-free with an '
                'MCV/Construction Yard. Other factions remain locked.'
            )
