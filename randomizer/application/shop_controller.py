...
from randomizer.core.paths import DTA_PUZZLE_PATH as ARCHIPELAGO_CAMEO_PATH
...
class ShopController(ShopPolishController):
    def initialize_shop_controller(self):
        ...
        self.shop_difficulty_var = tk.StringVar(value='Run Difficulty: +0')
    ...
    def _shop_modifier_clone_seed_plan(self, run):
        ...
        for target_id in active_shop_tech_ids(run):
            ...
            armor_reward = next((
                ...
                if str(reward.get('unit') or '').upper() == target_id
                and reward.get('buff_type') == 'armor'
            ), None)
            if armor_reward is not None:
        ...
        if self.shop_launch_active():
            ...
            if effects['player_armor_percent'] != 1:
                ...
                for target_id in armor_seeds:
                    reward = dict(next(
                        canonical_reward(item) for item in REWARD_POOL
                        if str(item.get('unit') or '').upper() == target_id
                        and item.get('buff_type') == 'armor'
                    ))
    ...
    def active_enemy_scaling_entries(self):
        if self.shop_launch_active():
            ...
            for _index in range(max(0, effects['enemy_armor_stacks'])):
                entries.append({
                    'reward': canonical_reward_for_id('Enemy Armor I'),
    ...
    def active_standard_starter_families(self):
        run = self._shop_context_run()
        if run is not None:
            faction = self.shop_run_faction_filter(run)
            return {
                'GDI': ('gdi',),
                'Nod': ('nod',),
                'Allies': ('allies',),
                'Soviet': ('soviet',),
            }.get(faction, ('gdi', 'nod', 'allies', 'soviet'))
    ...
    def give_up_shop_run(self):
        ...
        try:
            ...
            self._set_shop_message(
                f'Gave up Shop run at stage {run.stage}. '
    ...
    def open_selected_permanent_power_buffs(self):
        selected = self.shop_permanent_power_tree.selection()
        if not selected:
            return
        reward_id = self._shop_permanent_power_rows.get(selected[0], '')
        if reward_id not in self.shop_profile.permanent_power_unlocks:
            return
        self.shop_permanent_search_var.set('')
        self.shop_permanent_power_buff_target_var.set(reward_id)
        active_run = bool(
            self.shop_run is not None
            and self.shop_run.status is RunStatus.ACTIVE
        )
        self._refresh_permanent_power_buffs(active_run)
        self.shop_permanent_tabs.select(
            self.shop_permanent_power_buffs_panel
        )

    def show_shop_permanent_powers(self):
        self.shop_permanent_tabs.select(self.shop_permanent_powers_panel)
    ...
    def toggle_shop_setup_unit(self, event):
        ...
        if reward_id in self._shop_pending_loadout_selection:
            ...
            self._set_shop_message(
                ...
                f'{self._maximum_shop_loadout()} extra units or powers.',
                error=True,
            )
            return 'break'
        else:
    ...
    def _confirm_shop_mode_rules(self):
        ...
        if not messagebox.askokcancel(
            ...
            'At each stage, choose one '
            'mission. That selected mission gets one attempt.\n\nDo not save, '
            'load, or restart a mission. Any '
            'of these actions, a defeat, or closing the game before victory '
            'counts as a failed mission and can end the run. An Emergency '
            'Revival keeps the run alive by repeating the same stage with new '
            'mission choices.\n\n'
    ...
    def start_shop_run(self):
        ...
        starting_unit_markers = tuple(TIER_ONE_ROLE_MARKERS.values())
        starting_defense_markers = (TIER_ONE_DEFENSE_MARKER,)
        ...
        unit_entitlements = set(self.shop_profile.permanent_unit_unlocks)
        power_entitlements = set(self.shop_profile.permanent_power_unlocks)
        ...
        permanent_powers = tuple(
            reward_id for reward_id in self._selected_loadout_reward_ids()
            if reward_id in power_entitlements
            for entry in [self._shop_entry_by_reward_id.get(reward_id)]
            if entry is not None and self._shop_entry_available(entry)
        )
        ...
        try:
            ...
            self.shop_service.start_run(
                ...
                selected_reward_ids=selected_units,
    ...
    def _refresh_shop_setup(self):
        ...
        eligible_local_owned = {
            reward_id for reward_id in eligible_local_owned
        ...
        self.shop_loadout_help_var.set(
            ...
            + 'Permanent units and powers each use one slot. '
    ...
    def _refresh_permanent_shop(self):
        ...
        self._refresh_permanent_buffs(active_run)
        self._refresh_permanent_power_buffs(active_run)
        self.refresh_permanent_power_button()
    ...
    def _refresh_permanent_power_buffs(self, active_run):
        ...
        selected_reward_id = self.__dict__.pop(
            ...
            self._shop_permanent_power_buff_rows.get(
    ...
    def refresh_permanent_power_button(self, _event=None):
        ...
        owned_selected = bool(
            selected
            and self._shop_permanent_power_rows.get(selected[0], '')
            in self.shop_profile.permanent_power_unlocks
        )
        self.shop_permanent_power_buffs_button.configure(
            state='normal' if owned_selected else 'disabled'
        )
        if not selected:
        ...
        self.shop_permanent_power_info_var.set(
            ...
            'Permanent powers can be selected in future starting loadouts.'
        )
        self.shop_permanent_power_button.configure(
    ...
    def refresh_permanent_power_buff_button(self, _event=None):
        ...
        allowed = bool(
            selected
            and self._shop_permanent_power_buff_buyable.get(
                selected[0], False
            )
        )
    ...
    def buy_selected_permanent_power(self, _event=None):
        selected = self.shop_permanent_power_tree.selection()
        if not selected:
            return
        reward_id = self._shop_permanent_power_rows.get(selected[0])
    ...
    def buy_selected_permanent_buff(self, _event=None):
        selected = self.shop_permanent_buff_tree.selection()
        if not selected:
            return
    ...
    def buy_selected_permanent_power_buff(self, _event=None):
        selected = self.shop_permanent_power_buff_tree.selection()
        if not selected:
            return
